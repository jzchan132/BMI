...
class LanguageModel(BaseModel):
    def __init__(self, arch: str, dataset: str, optimizer: DictConfig, scheduler: DictConfig, annealing: DictConfig,
    ...
    def forward(self, input_ids, attention_mask):
        if self.p.dataset == 'cose_inhouse':
        ...
        if self.attn_reg:
            if self.attn_reg_layers == 'last':
                # (bsz, #layers=1, #heads, #tokens, #tokens)
                attns = outputs['attentions'][-1].unsqueeze(dim=1)
            elif self.attn_reg_layers == 'all':
                attns = torch.stack(outputs['attentions'], dim=1)
            else:
                # NOTE: expects str: layer=n
                layer = int(self.attn_reg_layers.split('=')[1])
                attns = outputs['attentions'][layer].unsqueeze(dim=1)
            return logits, attns
        elif self.p.multitask:
            sequence_output = outputs.hidden_states[-1]
            sequence_output = self.dropout(sequence_output)
            expl_logits = self.proj(sequence_output)
            return logits, expl_logits
        elif self.kd or self.save_outputs:
            # (bsz, #layers, #heads, #tokens, #tokens)
            attns = torch.stack(outputs['attentions'], dim=1)[:, :, :, 0, :]
            if len(self.kd_attn_layers) > 0 and not self.save_outputs:
                attns = attns[:, self.kd_attn_layers.to(self.device), :, :]
